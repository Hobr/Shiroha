...
1. [`architecture.md`](architecture.md) — 顶层架构、设计原则、**跨切面契约**(幂等性/优雅关闭/背压/安全)
...
- [`roadmap.md`](roadmap.md) — 版本分期与里程碑(v0.3.0 → v1.0.0)

## 跨切面关注点

以下关注点横跨多个 crate,在 `architecture.md` 的"跨切面契约"一节中统一定义,各子模块文档交叉引用而非重复描述:

- **Action 幂等性** — 所有 Action 必须幂等;非幂等 Action 必须使用 WaitingMode=Waiting
- **优雅关闭** — SIGTERM 后的有序关闭流程
- **背压与并发控制** — Worker 并发上限与 Dispatcher 退避
- **WASM 沙箱安全** — Fuel / 内存 / 栈深度 / 网络超时限制
- **网络安全** — SSRF 防护、控制面鉴权、能力声明校验

## 写作原则

- 描述「是什么」与「为什么」,不写实现代码
- 跨 crate 的契约在双方文档中同时给出指向
- 决策一经敲定,直接落到对应模块文档;新决策点应当在 PR 描述中明确指向受影响的文件
- **不重复**:跨切面关注点只在 `architecture.md` 中定义一次,其他文件用一句交叉引用
