...
| v0.4.0 | 持久化 + 控制面 + CLI(单机完整可用) | storage / proto / control / config / apps |
| v0.5.0 | 分布式基础(远程节点,N=1 派发) | transport / transport-grpc / worker / dispatch |
...
- core 的 FSM / ActionRef / DispatchPolicy / Aggregation 类型与 trait
- 加载 WASM component,读取 `describe` / `decide` / `action` 导出
- 主机能力:`log` + `clock`(其它能力 v0.4+ 再加)
- 引擎在内存中跑 Job,主控关闭即丢
- 仅 `DispatchPolicy::Local` 路径(LocalExecutor 直接调 wasm)
- `WaitingMode = Blocking` only

**不在范围**:持久化、网络、CLI、远程节点、Waiting 模式、Quorum/Custom 聚合
...
**新增 crate**:`shiroha-storage`、`shiroha-proto`、`shiroha-control`、`shiroha-config`、`apps/shirohad`、`apps/sctl`

**新增能力**:

- Store trait + redb 实现;Component / Flow / Job / Event 持久化
...
**新增 crate**:`shiroha-transport`、`shiroha-transport-grpc`、`shiroha-worker`、`shiroha-dispatch`
...
- Dispatcher:`DispatchPolicy::Local` + `DispatchPolicy::Remote(selector=one(...), agg=First)`
...
- Aggregator 提前返回 + 沿 transport 取消 + 后台兜底(晚到结果写日志)
...
- Event 保留:Job 终态后 N 天清理(默认 30 天,配置项可调)
