...
无论哪种模式,从一次转移开始到完成,Storage 中的 Job 状态必须始终是可恢复的快照:主控随时崩溃,重启后都能从 Storage 推算出"上一次到哪一步"。Waiting 模式下,重启后 Engine 可以直接看到 `Waiting(action_id)` 并重新订阅该 action 的结果通道。**Blocking 模式下,主控重启时所有未持久化结果的 Action 一律视为失败**,走 ActionRef 声明的失败处理路径——如果该 Action 不能容忍这种重启失败,作者应当在 ActionRef 上声明 `WaitingMode = Waiting`。
