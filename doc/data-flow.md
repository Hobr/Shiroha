...
5. Engine 校验 WASM import 与 Flow 能力声明的一致性(拒绝声明之外的 import)
6. Engine 派生 ComponentId(内容 hash);Storage 在**单事务**内:若 ComponentId 已存在则只追加 Flow 引用,否则写入 Component 字节;同事务内写入 Flow 记录(引用 ComponentId)与能力声明
7. 写入完成;**不主动推送到节点**——节点会在首次需要执行该 ComponentId 上的 Action 时按需 pull(见 `worker.md`)
8. 返回给 sctl 的响应包含 Flow 标识 `(name, version)` 与对应的 ComponentId

任何一步失败都不允许留下半完成的 Flow——校验失败时,Storage 中无任何残留。
...
4. 若任一初始 Action 硬失败,Engine **显式回滚**:删除 Job 记录并 append `CreationFailed` Event(而非依赖事务回滚,因为事务可能已提交、且 Action 派发可能已产生外部副作用)
...
4. 正在该节点上的未完成 Action:心跳通道与结果通道独立,已到达的结果仍正常参与聚合;若 Aggregator 已提前返回,晚到结果进入 dead letter(见 `dispatch.md`)
...
5. 已经派发但未持久化结果的 Action:Waiting 模式直接订阅结果通道;**Blocking 模式一律视为失败,走 ActionRef 失败路径**(详见 `engine.md` 与 `architecture.md` 的"Action 幂等性"一节)
