...
7. 返回给 sctl 的响应包含 Flow 标识 `(name, version)` 与对应的 ComponentId

任何一步失败都不允许留下半完成的 Flow——校验失败时,Storage 中无任何残留;Storage 是字节的权威源,节点会在首次需要时按需 pull,不存在"字节分发失败"这一步。
...
5. 已经派发但未持久化结果的 Action:Waiting 模式直接订阅结果通道;**Blocking 模式一律视为失败,走 ActionRef 失败路径**(详见 `engine.md`)
