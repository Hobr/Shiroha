...
| 观测 | tail-events / get-job-state / job-event-history / export-events / list-dead-letters | 实时与历史查询;export-events 将事件导出为 JSON Lines;list-dead-letters 查询被丢弃的晚到结果 |
...
- **本地 UDS** — 受文件系统权限保护;可选 peer credential 检查(通过 `SO_PEERCRED` 获取连接进程的 UID),可配置允许访问的用户列表。详见 `architecture.md` 的"网络安全"一节
- **远程 TCP** — 需要 mTLS;客户端身份由证书决定;粗粒度授权(只读 / 读写 / 管理)在 MVP 后引入
- **令牌** — 长期未定,先用 mTLS 作为唯一鉴权机制

具体细节列入后续工作,不阻塞首个可用版本。v0.4 至 v0.7 期间控制面仅在受信网络内运行,不对外暴露;远程访问鉴权随 v0.8 mTLS 一并启用。

## 与其他 crate 的契约

- 依赖:`shiroha-core`、`shiroha-engine`、`shiroha-storage`(只读查询,通过 QueryStore)、`shiroha-proto-control`
