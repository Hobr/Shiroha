...
节点上不持有任何 Job 状态。重启节点不需要恢复任何业务上下文(WASM 组件缓存除外,而缓存丢失只是重新拉取一次的代价)。

## 主要职责

- 启动时向主控声明自己(endpoint、能力标签、版本信息)
- 维持组件缓存:已知 `(FlowId, Version)` 对应的 WASM 字节
...
节点对每个 `(FlowId, Version)` 缓存一份 WASM 字节。缓存如何获得:

- **预加载** — 主控在 Flow 上传时把字节推给当前注册的所有节点
- **按需 pull** — 节点收到 Action 请求时若本地无缓存,向主控发起一次拉取

两种策略各有取舍,见 `open-questions.md`。无论选择哪种,主控始终是字节的权威源,节点缓存可丢可重建。
