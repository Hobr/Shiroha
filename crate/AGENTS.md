...
Workspace 库 crate 目录。当前包含 8 个独立 crate，按关注点分离并通过 trait / shared WIT 抽象解耦。
...
| `shiroha-wit/` | canonical WIT 定义，供 SDK build script、示例和宿主侧测试共享 |
...
- 变更 WIT 接口时，同时检查 `shiroha-wit`、`shiroha-sdk`、示例 component 和宿主侧调用
...
shiroha-wit (独立，提供 canonical WIT 定义)
  ↑
shiroha-sdk (面向 guest 侧；build-dependency 依赖 shiroha-wit，运行时暴露宏和 helper)
```
