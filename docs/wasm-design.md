...
| 接口 | 调用方 | 说明 |
| ------ | -------- | ------ |
| `invoke-action(name, context)` | Controller / Node | 执行指定 Action，传入 job-id、当前状态、payload；返回执行状态和输出 |
...
- 但这仍是 component 级能力边界，不是 action 级权限系统；更细粒度的 capability 约束仍属于后续迭代
