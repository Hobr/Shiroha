...
| `on-enter` | 可选，进入状态时执行的 action 函数名 |
| `on-exit` | 可选，离开状态时执行的 action 函数名 |
...
| 接口 | 调用方 | 说明 |
| ------ | -------- | ------ |
| `invoke-action(name, context)` | Node | 执行指定 Action，传入 job-id、当前状态、payload；返回执行状态和输出 |
| `invoke-guard(name, context)` | Controller | 评估 Guard 条件，传入 job-id、from/to 状态、事件、payload；返回 bool |
...
| `network` | HTTP 客户端（domain allowlist） | 需要调用外部 API 的 Action |
...
- **预编译缓存**：部署时序列化编译产物，运行时反序列化，跳过编译开销
- **版本管理**：Job 创建时绑定 Flow 版本，多版本 WASM 可共存
- **Node 端缓存**：Controller 维护模块 registry，Node 按 content hash 缓存，任务只下发模块 ID + 函数入口
