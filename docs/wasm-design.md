...
- 当前 runtime 还会按 `action.capabilities` 在每次调用时动态放行 network / storage capability
- 因此 `host-world` 是上界，真正的 capability 放行粒度当前已经下沉到单个 action
...
| `remote` | 分发到单个 Node 执行；当前 standalone 会通过同进程 node worker 执行 |
...
| 接口 | 调用方 | 说明 |
| ------ | -------- | ------ |
| `supports-action(name)` | Controller | deploy 期校验指定 action 名是否受 guest 支持 |
| `supports-guard(name)` | Controller | deploy 期校验指定 guard 名是否受 guest 支持 |
| `supports-aggregate(name)` | Controller | deploy 期校验指定 aggregator 名是否受 guest 支持 |
...
- host 使用 `wasmtime::component` typed exports 调用 `get-manifest` / `supports-*` / `invoke-action` / `invoke-guard` / `aggregate`
...
- 当前 deploy 会验证通用导出接口存在，并通过 `supports-action` / `supports-guard` / `supports-aggregate` 校验 manifest 中命名的引用是否受 guest 支持
...
当前 standalone 实现里，`remote` 已经会通过 in-process transport 进入同进程的 node worker，再由该 worker 调用 guest；它不再与 `local` 共用完全相同的调用边界。
...
- 这还不是一个完整的 network policy 层；当前没有 domain allowlist，也没有按 flow / action 细分的出站规则
- guest 仍可配置 proxy、本地绑定地址、自定义 root certificate 以及危险 TLS 选项
- 因此 `network` world 当前更像“获得 HTTP 能力的 capability”，还不是严格的安全边界
...
- 因此 `storage` world 当前表示“允许直接访问宿主 KV capability”，不是隔离后的租户边界
...
- standalone 运行时已经会执行 fan-out：分发到同进程的 fan-out 槽位、收集结果、调用 guest `aggregate()`，并把返回事件作为后续内部事件继续推进状态机
- 当前这仍不是“真实多节点集群 fan-out”：`all` / `count(N)` / `tagged(...)` 在 standalone 里只会映射到同进程槽位或标签名，不依赖节点注册/发现
- guest `aggregate()` 返回的事件必须能从 fan-out action 提交后的目标状态继续匹配转移；否则后续推进会失败
...
- Flow 删除时会移除 Flow 注册记录，但当前不会对原始 wasm bytes 或 `module_cache` 做引用计数/共享生命周期管理
- 也就是说，删除 Flow 之后相关 wasm 产物当前可能继续保留，后续回收策略仍属于后续迭代
