...
├── wasm-engine     WASM 运行时封装 + WIT 绑定 + 能力注入 + 权限守卫
├── fsm             状态机引擎，纯逻辑，Effect 模式，不感知外部世界
├── dispatch        分发 + 聚合，含本地执行和远程执行两条路径
├── transport       传输层抽象 + gRPC 实现 + Protobuf 定义
└── storage         持久化抽象 + 嵌入式存储实现

app/
├── server          统一入口，按配置激活 Controller / Node / Standalone
└── cli             CLI 客户端，通过管理接口与 Server 交互
```
